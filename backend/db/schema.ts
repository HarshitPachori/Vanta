...
export const users = sqliteTable(
	'users',
	{
		...
		hasGmailAccess: integer('has_gmail_access', { mode: 'boolean' }).notNull().default(false),
...
export const senders = sqliteTable(
	...
	(t) => [index('senders_user_id_idx').on(t.userId), uniqueIndex('senders_user_email_idx').on(t.userId, t.email)],
);
...
export const unsubscribeJobs = sqliteTable(
	...
	(t) => [index('unsub_jobs_user_id_idx').on(t.userId), index('unsub_jobs_status_idx').on(t.status)],
);
...
export const subscriptions = sqliteTable(
	...
	(t) => [uniqueIndex('subscriptions_user_id_idx').on(t.userId), uniqueIndex('subscriptions_stripe_customer_idx').on(t.stripeCustomerId)],
);
...
export const passwordResetTokens = sqliteTable(
	...
	(t) => [index('prt_user_id_idx').on(t.userId), uniqueIndex('prt_token_idx').on(t.token)],
);
...
export const auditLogs = sqliteTable(
	'audit_logs',
	{
		...
		userId: text('user_id')
			...
			.references(() => users.id, { onDelete: 'cascade' }),
		action: text('action', {
			enum: ['scan_started', 'scan_done', 'scan_failed', 'unsub_requested', 'unsub_done', 'unsub_failed', 'digest_add', 'digest_sent'],
		}).notNull(),
