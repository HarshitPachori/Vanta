...
export const getValidAccessToken = async (
	...
	const refreshed = await refreshAccessToken(user.gmailRefreshToken, env.GOOGLE_CLIENT_ID, env.GOOGLE_CLIENT_SECRET);
...
export const listMessageIds = async (accessToken: string, query: string, maxResults = 500): Promise<string[]> => {
	const ids: string[] = [];
...
export const batchGetMessageHeaders = async (accessToken: string, messageIds: string[]): Promise<GmailMessageDetail[]> => {
...
export type DigestMessageDetail = {
	id: string;
	from: string;
	subject: string;
	date: string;
...
export const batchGetMessagesForDigest = async (
	accessToken: string,
	messageIds: string[],
	maxBodyChars = 500,
): Promise<DigestMessageDetail[]> => {
	const CHUNK = 20; // smaller chunks — format=full is heavier
	const results: DigestMessageDetail[] = [];
	...
	for (let i = 0; i < messageIds.length; i += CHUNK) {
		...
		const fetches = chunk.map((id) =>
			fetch(`${GMAIL_BASE}/messages/${id}?format=full`, {
				headers: { Authorization: `Bearer ${accessToken}` },
			}).then((r) => (r.ok ? r.json<GmailMessageFull>() : null)),
		);
		...
		for (const s of settled) {
			...
			results.push({
				...
				subject,
				date,
				body: rawText.slice(0, maxBodyChars).trim(),
			});
		}
...
export const getHeader = (headers: GmailMessageHeader[], name: string): string | undefined =>
	headers.find((h) => h.name.toLowerCase() === name.toLowerCase())?.value;

export const parseFrom = (fromHeader: string): { email: string; displayName: string } => {
...
export const parseUnsubscribeHeader = (header: string): { mailto?: string; http?: string } => {
...
export const archiveMessages = async (accessToken: string, messageIds: string[]): Promise<void> => {
	const CHUNK = 50;
	for (let i = 0; i < messageIds.length; i += CHUNK) {
		const chunk = messageIds.slice(i, i + CHUNK);
...
export const createGmailFilter = async (accessToken: string, senderEmail: string): Promise<boolean> => {
