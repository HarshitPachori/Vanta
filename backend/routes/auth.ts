...
const createSession = async (db: ReturnType<typeof getDb>, userId: string, jwtSecret: string, meta: SessionMeta = {}) => {
...
auth.post('/forgot-password', zValidator('json', forgotSchema), async (c) => {
	...
	await sendPasswordResetEmail(user.email, resetUrl, c.env.RESEND_API_KEY, c.env.RESEND_FROM_EMAIL);
...
auth.post('/reset-password', zValidator('json', resetSchema), async (c) => {
	...
	await db.update(users).set({ passwordHash, updatedAt: ts }).where(eq(users.id, resetToken.userId));

	// mark token used
	await db.update(passwordResetTokens).set({ usedAt: ts }).where(eq(passwordResetTokens.id, resetToken.id));
...
auth.get('/google/callback', async (c) => {
	...
	try {
		...
		if (connectingUserId) {
			...
			return c.redirect(`${c.env.CLIENT_BASE_URI}/dashboard/settings?gmail=connected`);
		}

		let user = await db.select().from(users).where(eq(users.email, profile.email)).get();
		...
		if (user) {
			...
			user = await db.select().from(users).where(eq(users.email, profile.email)).get();
		}
		...
		return c.redirect(scanDone ? `${c.env.CLIENT_BASE_URI}/dashboard` : `${c.env.CLIENT_BASE_URI}/onboarding`);
	} catch (err) {
