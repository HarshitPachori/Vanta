...
export function SignupForm() {
	...
	return (
		<motion.div variants={fadeUp()} initial="hidden" animate="show" className="flex flex-col gap-4">
			...
			<form onSubmit={handleSubmit} noValidate aria-label="Sign up form" className="flex flex-col gap-4">
				{error && (
					<motion.div
				...
				<div className="flex flex-col gap-1.5">
					<Label htmlFor="name" className="text-sm text-(--color-text-secondary)">
						Name
					</Label>
					<div className="relative">
						<User aria-hidden="true" size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-(--color-text-muted)" />
						<Input
							id="name"
							name="name"
							type="text"
