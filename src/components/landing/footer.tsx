      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8 py-12">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-10">
          ...
          <div className="col-span-2">
            ...
            <p className="mt-5 text-xs text-(--color-text-muted) font-mono">
              © {new Date().getFullYear()} Vanta
            </p>
          </div>
          ...
          {Object.entries(FOOTER_LINKS).map(([group, links]) => (
            <nav key={group} aria-label={`${group} links`}>
              <h3 className="text-xs font-mono text-(--color-text-muted) uppercase tracking-widest mb-4">
                {group}
              </h3>
