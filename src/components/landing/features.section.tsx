		...
		<section id="features" aria-labelledby="features-heading" className="py-24 px-5">
			<div className="mx-auto max-w-6xl">
				<header className="mb-16">
					<p className="text-xs font-mono text-(--color-accent) uppercase tracking-[0.12em] mb-3">Features</p>
					<h2
						id="features-heading"
				...
				>
					{FEATURES.map(({ icon: Icon, num, title, description }) => (
						<motion.li key={title} variants={fadeUp(0)}>
							<article
								className={cn(
							...
							>
								...
								<h3 className="font-display font-bold text-sm text-(--color-text-primary) mb-2 tracking-tight">{title}</h3>
