...
export default function HeroAnimation() {
	...
	useEffect(() => {
		...
		const t = setTimeout(() => setVisibleCount((v) => v + 1), 350 + visibleCount * 120);
	...
	return (
		<figure
			...
			className={cn(cardSection, 'border-(--color-accent-border) shadow-(--shadow-glow) overflow-hidden rounded-2xl')}
		>
			{/* Window chrome */}
			<header className="flex items-center gap-2 px-4 py-3 border-b border-(--color-border) bg-(--color-surface-2)">
				<span aria-hidden="true" className="h-2.5 w-2.5 rounded-full bg-red-500/60" />
				<span aria-hidden="true" className="h-2.5 w-2.5 rounded-full bg-amber-500/60" />
				<span aria-hidden="true" className="h-2.5 w-2.5 rounded-full bg-green-500/60" />
				<span className="ml-3 text-xs text-(--color-text-muted) font-mono tracking-wide">vanta — inbox scan</span>
				<span className="ml-auto flex items-center gap-1.5">
					<span className="h-1.5 w-1.5 rounded-full bg-(--color-accent) animate-pulse" aria-hidden="true" />
					<span className="text-xs font-mono text-(--color-accent)">scanning</span>
				</span>
			</header>
			...
			>
				{STATS.map(({ label, value }) => (
					<div key={label} className="flex flex-col items-center py-3 px-4">
						<span className="text-lg font-bold font-display text-(--color-text-primary) tracking-tight">{value}</span>
						<span className="text-xs text-(--color-text-muted) mt-0.5">{label}</span>
					</div>
				))}
			</div>
			...
			<ul aria-label="Email list being processed" className="min-h-72">
						...
						>
							...
							<span
								...
								className={cn(
									...
									email.keep ? 'bg-(--color-accent-muted) text-(--color-accent)' : 'bg-(--color-surface-3) text-(--color-text-muted)',
								)}
							>
								{email.from[0]}
							</span>
							...
							<div className="flex-1 min-w-0">
								<p className="text-sm font-medium text-(--color-text-primary) truncate leading-tight">{email.from}</p>
								<p className="text-xs text-(--color-text-muted) truncate mt-0.5">{email.subject}</p>
							</div>
							...
							<motion.span
								...
								className={cn(
									...
									email.keep ? 'text-(--color-success)' : 'text-(--color-danger)',
								)}
							>
								{email.keep ? <Inbox size={12} aria-hidden="true" /> : <Trash2 size={12} aria-hidden="true" />}
				...
				{visibleCount < EMAILS.length &&
					Array.from({ length: Math.min(2, EMAILS.length - visibleCount) }).map((_, i) => (
						<li
							key={`skeleton-${i}`}
							className={cn(skeleton, 'flex items-center gap-3 px-4 py-3 border-b border-(--color-border) last:border-b-0')}
						>
			...
			<footer className="flex items-center justify-between px-4 py-2.5 border-t border-(--color-border) bg-(--color-surface-2)/50">
				<div className="flex items-center gap-1.5">
					<Mail size={11} className="text-(--color-text-muted)" aria-hidden="true" />
					<span className="text-xs font-mono text-(--color-text-muted)">
						{visibleCount} of {EMAILS.length} processed
					</span>
				</div>
				<div className="flex items-center gap-3">
					<span className="text-xs font-mono text-(--color-danger)">3 unsubscribed</span>
					<span className="text-xs font-mono text-(--color-success)">3 in digest</span>
				</div>
			</footer>
		</figure>
	);
