...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  promo:
    "bg-(--color-warning-muted) text-(--color-warning) border-amber-700/20",
  social:
    "bg-(--color-surface-3) text-(--color-text-muted) border-(--color-border)",
  transactional:
    "bg-(--color-success-muted) text-(--color-success) border-(--color-success)/20",
...
export default function DigestClient({
  ...
  const [frequency, setFrequency] = useState<"daily" | "weekdays" | "weekly">(
    (initialDigest?.frequency as never) ?? "daily",
  );
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <aside className="lg:col-span-1 flex flex-col gap-4">
          ...
          <section
            aria-labelledby="digest-config-heading"
            className={cn(cardSection," rounded-xl p-5 flex flex-col gap-4")}>
            <h2
            ...
            <div className="flex flex-col gap-1.5">
              <Label
                htmlFor="digest-time"
                className="text-xs text-(--color-text-muted) flex items-center gap-1.5">
          ...
          <div className={cn(metricCard,"rounded-xl p-4 flex items-center justify-between")}>
            <div>
              ...
              <p className="font-display font-black text-2xl text-(--color-accent) leading-none mt-1">
                {selectedIds.size}
              </p>
            </div>
        ...
        {/* Right — sender picker */}
        <section
          aria-labelledby="sender-picker-heading"
          className="lg:col-span-2 flex flex-col gap-3">
          <div className="flex items-center justify-between gap-3">
            <h2
              id="sender-picker-heading"
