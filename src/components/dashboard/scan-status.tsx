	...
	return (
		<AnimatePresence mode="wait">
			<motion.section
				...
				className={cn(cardSection, ' rounded-xl p-5 flex items-center gap-4', status === 'scanning' && 'border-(--color-accent-border)')}
			>
				{/* Icon */}
				<span
					aria-hidden="true"
				...
				<div className="flex-1 min-w-0">
					<h2 id="scan-status-heading" className="font-display font-bold text-sm text-(--color-text-primary) tracking-tight">
				...
				{/* Action */}
				{status === 'scanning' && <span className="text-xs font-mono text-(--color-accent) animate-pulse shrink-0">Live</span>}
				{(status === 'idle' || status === 'failed') && (
					<Button size="sm" onClick={triggerScan} disabled={pending} className="shrink-0">
						{pending ? <Loader size={13} className="animate-spin" /> : 'Scan now'}
					</Button>
				)}
				{status === 'done' && (
					<Button size="sm" variant="outline" onClick={triggerScan} disabled={pending} className="shrink-0 gap-1.5">
						{pending ? <Loader size={13} className="animate-spin" /> : <RefreshCw size={13} />}
						Rescan
					</Button>
				)}
