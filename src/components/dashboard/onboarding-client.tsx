...
export default function OnboardingClient({ userName, hasGmailAccess: initialGmail, scanStatus: initialScan }: Props) {
	...
	return (
		<div className="w-full max-w-lg">
			...
			<motion.div variants={fadeUp()} initial="hidden" animate="show" className="text-center mb-10">
				...
				<p className="text-sm text-(--color-text-muted) mt-2">Three quick steps to inbox zero.</p>
			</motion.div>
							...
							{i < STEPS.length - 1 && (
								<div
									className={cn(
										...
										i < currentStepIndex ? 'bg-(--color-success)' : 'bg-(--color-border)',
									)}
									aria-hidden="true"
								/>
							)}
			...
			<AnimatePresence mode="wait">
				...
				{step === 'scan' && (
					...
					>
						...
						{(scanStatus === 'idle' || scanStatus === 'failed') && (
							<Button size="lg" className="w-full shadow-(--shadow-glow-sm)" onClick={triggerScan} disabled={pending}>
								{pending ? <Loader size={16} className="animate-spin" aria-hidden="true" /> : <ScanLine size={16} aria-hidden="true" />}
				...
				{step === 'done' && (
					...
					>
						...
						<div>
							<h2 className="font-display font-bold text-xl text-(--color-text-primary) tracking-tight">Inbox scanned</h2>
							<p className="text-sm text-(--color-text-muted) mt-2 leading-relaxed">
								Found <span className="font-bold text-(--color-text-primary)">{senderCount.toLocaleString()}</span> senders. Now let's clean
								things up.
							</p>
						</div>
