...
const getData = async () => {
	try {
		...
		const [[totalRow], [unsubRow], [digestRow], [newsletterRow]] = await Promise.all([
			db.select({ count: count() }).from(senders).where(eq(senders.userId, user.id)),
			db
				.select({ count: count() })
				.from(senders)
				.where(and(eq(senders.userId, user.id), eq(senders.status, 'unsubscribed'))),
			db
				.select({ count: count() })
				.from(senders)
			...
			db
				.select({ count: count() })
				.from(senders)
				.where(and(eq(senders.userId, user.id), eq(senders.category, 'newsletter'))),
		]);
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ gmail?: string }> }) {
	...
	return (
		<div className="px-5 sm:px-8 py-8 max-w-4xl mx-auto">
			...
			<header className="mb-8">
				<h1 className="font-display font-bold text-2xl text-(--color-text-primary) tracking-tight">Overview</h1>
				<p className="text-sm text-(--color-text-muted) mt-1">Your inbox health at a glance.</p>
			</header>
			...
			{!user.hasGmailAccess && (
				<section
					...
					className={cn(cardSection, 'rounded-2xl p-8 flex flex-col items-center text-center gap-5 mb-8')}
				>
					<span
						aria-hidden="true"
					...
					<div>
						<h2 id="connect-heading" className="font-display font-bold text-lg text-(--color-text-primary) tracking-tight">
							Connect your Gmail
						</h2>
					...
					<p className="text-xs text-(--color-text-muted) font-mono">Read-only · No content stored · Revoke anytime</p>
				</section>
			)}

			{/* Scan status + trigger */}
			{user.hasGmailAccess && <ScanStatus scanStatus={user.scanStatus} lastScannedAt={user.lastScannedAt} />}
				...
				<>
					<dl className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-8">
						{STATS.map(({ label, value, icon: Icon, color }) => (
							<div key={label} className={cn(metricCard, 'rounded-xl p-5 flex flex-col gap-2')}>
								<div className="flex items-center justify-between">
									<dt className="text-xs text-(--color-text-muted) font-mono">{label}</dt>
									<Icon size={14} aria-hidden="true" className={color} />
								</div>
								<dd className={`font-display font-black text-3xl tracking-tight leading-none ${color}`}>{value.toLocaleString()}</dd>
							</div>
						))}
					</dl>
					...
					>
						...
						>
							...
							>
								<Trash2 size={16} className="text-(--color-accent) group-hover:text-white transition-colors" />
							</span>
							<div>
								<h3 className="font-display font-bold text-sm text-(--color-text-primary) tracking-tight">Manage senders</h3>
								<p className="text-xs text-(--color-text-muted) mt-1 leading-relaxed">Review all detected senders and bulk unsubscribe.</p>
							</div>
						</Link>
						...
						>
							...
							>
								<Layers size={16} className="text-(--color-accent) group-hover:text-white transition-colors" />
							</span>
							<div>
								<h3 className="font-display font-bold text-sm text-(--color-text-primary) tracking-tight">Set up digest</h3>
								<p className="text-xs text-(--color-text-muted) mt-1 leading-relaxed">Pick newsletters and get one clean daily email.</p>
							</div>
						</Link>
					</section>
				</>
