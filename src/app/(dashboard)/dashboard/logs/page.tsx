...
const getDetails = (action: AuditAction, raw: string | null): string => {
	...
	try {
		...
		switch (action) {
			case 'scan_done':
				return `Found ${Number(m.senderCount).toLocaleString()} senders`;
			case 'scan_failed':
				return `Reason: ${m.reason ?? 'unknown'}`;
			case 'unsub_requested':
				return `${m.count} sender${m.count !== 1 ? 's' : ''} queued`;
			case 'unsub_done':
				return m.displayName ? `${m.displayName} <${m.email}>` : String(m.email ?? '—');
			case 'unsub_failed':
				return `${m.email} — ${m.error ?? 'unknown error'}`;
			case 'digest_add':
				return `${m.count} sender${m.count !== 1 ? 's' : ''} added`;
			case 'digest_sent':
				return `${m.digestName} — ${m.itemCount} email${m.itemCount !== 1 ? 's' : ''}`;
			default:
				return '—';
		}
	} catch {
...
const getData = async () => {
	try {
		...
		return db.select().from(auditLogs).where(eq(auditLogs.userId, user.id)).orderBy(desc(auditLogs.createdAt)).limit(100).all();
	} catch {
		return null;
	}
};
	...
	return (
		<div className="px-5 sm:px-8 py-8 max-w-4xl mx-auto">
			<header className="mb-8">
				<h1 className="font-display font-bold text-2xl text-(--color-text-primary) tracking-tight">Activity</h1>
				<p className="text-sm text-(--color-text-muted) mt-1">Audit log of all actions — last 100 entries, auto-cleared after 48 hours.</p>
			</header>
			...
			{logs.length === 0 ? (
				...
				<div className={cn(cardSection, 'overflow-hidden')}>
					...
					<div className="px-4 py-2.5 border-t border-(--color-border)">
						<p className="text-[11px] font-mono text-(--color-text-muted)">{logs.length} entries</p>
					</div>
				</div>
			)}
		</div>
